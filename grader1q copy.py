...
def extrair_nota_texto(texto: str, q_weight: int = 100) -> str:
    ...
    Estratégias em ordem de confiabilidade:
      1. "Nota: X + Y + Z = NN/100"  — soma explícita ao final
      2. "Nota final (Q1)\n\nNota: NN/100"
      3. "NN/100" — fração sobre o peso máximo
      4. Linha "Nota: NN/100" ou "Total: NN"
      5. Último número inteiro plausível no texto
    """
    # Estratégia 1: "X + Y + Z = NN/100"  (linha de soma final)
    m = re.search(
    ...
    # Estratégia 2: "Nota final" seguida de "Nota: NN/100" em até 3 linhas
    m = re.search(
    ...
    # Estratégia 4: "Nota: NN" ou "Total: NN" em linha própria
    m = re.search(
        r'(?:^|\n)[^\n]*(?:nota|total)[^\n]*?([0-9]+(?:[.,][0-9]+)?)\s*(?:/|pontos|pts)',
        texto, re.IGNORECASE
    )
    if m:
        val = float(m.group(1).replace(',', '.'))
...
def main():
    ...
    parser.add_argument("base_dir",       help="Pasta raiz das submissões (ex: Simulado0)")
